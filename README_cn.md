...
📖 Language: **中文**（机器翻译） | [English](README.md)
...
*   **JDK 版本:** **强烈建议使用 Java 8 (1.8)**。
    *   *开发环境:* Java 1.8.0_281。
    *   *兼容性提醒:* 如果你用的是 JDK 11/17/21 等高版本，可能会出现动画忽快忽慢或者图片显示等瑕疵，不过可以运行。

### 2. 导入项目
为了防止图片加载不出来，请务必按照以下步骤打开：
1.  打开 IntelliJ IDEA。
2.  直接 clone 这个仓库并用idea里面的Project打开。
...
1.  **100%基于 Swing 的动画:** 我们完全基于 `JFrame` 实现了非常丝滑的动画效果。这不需要使用额外的游戏引擎，也不用超纲的知识，只用 JavaA 学的内容也能做出很好的视觉效果。
2.  **精致的 UI 设计:** 我们的游戏外观非常精致，或许能给你做 UI 设计时提供一点思路。
